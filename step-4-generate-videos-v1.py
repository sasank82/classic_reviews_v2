...
def find_relevant_segments(transcript, keywords):
    segments = []
...
def extract_video_clips(video_path, segments):
    video_clips = []
    video = VideoFileClip(video_path)
    for segment in segments:
        start_time = segment['start']
        end_time = segment['end']
        video_clips.append(video.subclip(start_time, end_time))
    return video_clips
...
def create_subscene_with_videos(subscene, section, audio_clip, device_name, video_clips):
    try:
        logging.info(f"Creating subscene: {subscene}")
        subscene_duration = subscene['end_time'] - subscene['start_time']
        canvas = mpy.ColorClip(size=(1280, 720), color=(128, 128, 128)).set_duration(subscene_duration)
        ...
        for video_clip in video_clips:
            try:
                clip = video_clip.set_duration(min(video_clip.duration, subscene_duration))
                clips_video.append(clip)
            except Exception as e:
                logging.error(f"Error processing video clip: {e}")
                continue

        video_composite_clip = mpy.CompositeVideoClip(clips_video, size=(1280, 720)).set_fps(24)

        # Process Overlays (same as before)
        clips_overlay = []  # Your existing overlay processing code here

        overlay_composite_clip = mpy.CompositeVideoClip(clips_overlay, size=(1280, 720)).set_fps(24)

        subscene_clip = mpy.CompositeVideoClip([video_composite_clip, overlay_composite_clip], size=(1280, 720), use_bgclip=True).set_fps(24).set_start(subscene['start_time']).set_duration(subscene_duration)
...
def process_scene(scene, audio_clip, brand_name, device_name, index, language_code, keywords):
    ...
    for subscene in subscene_data:
        ...
        try:
            relevant_segments = find_relevant_segments(transcript, keywords)
            video_clips = extract_video_clips(video_path, relevant_segments)
            subscene_clip = create_subscene_with_videos(subscene, scene, audio_clip, device_name, video_clips)
        except Exception as e:
            logging.error(f"Error creating subscene: {e}")
            subscene_clip = None
